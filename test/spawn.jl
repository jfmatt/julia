...
isunix() && run(pipeline(yescmd, `head`, DevNull))
...
# Stream Redirection
if !iswindows() # WINNT reports operation not supported on socket (ENOTSUP) for this test
    local r = Channel(1)
...
let out = Pipe(), echo = `$exename --startup-file=no -e 'print(STDOUT, " 1\t", readstring(STDIN))'`, ready = Condition(), t, infd, outfd
    ...
    t = @async begin # spawn writer task
        ...
        close(out.in)
        @test !isopen(out.in)
        iswindows() || @test !isopen(out.out) # it takes longer to propagate EOF through the Windows event system
